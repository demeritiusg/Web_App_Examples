...
writer = pd.ExcelWriter('volumes_report.xlsx', engine='xlsxwriter', date_format='YYYY-MM')
sheet_name = 'Chart'
...
for p in product_name:
    ...
    totals_rev_prod = df.groupby(['production_date'])['owner_net_value'].sum().reset_index()  # correct
...
price_table = pd.pivot_table(df, values='resolved_price', index='production_date', columns='simple_product_name',
                             aggfunc=np.mean).round(2).fillna('-')

df_pivot = vols_table.merge(price_table, on='production_date').reset_index().merge(totals_rev_prod,
                                                                                   on='production_date').merge(
    totals_rev_acct, on='production_date').fillna('-')

df_pivot['prod_date'] = df_pivot['production_date'].dt.strftime('%Y-%m')

df_pivot = df_pivot[['prod_date', 'prod_month', 'acct_month', 'oil_boe', 'oil_x',
